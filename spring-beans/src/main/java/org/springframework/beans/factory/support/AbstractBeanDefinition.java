...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 依赖是一个simple属性，基本数据类型、包装类、枚举、Number、Date等类型
	...
	/**
	 * 容器应该尝试去推断Bean的destroy方法，而不是显示的规范方法。该值用专门用于在方法名称中，否则为非法
	 * 确保拥有相同合法名称的方法名不会冲突。
	 ...
	 * 作用域，默认为空字符串
	 *
	 * @see BeanDefinition#SCOPE_SINGLETON 一个Spring容器中，一个BeanDefinition只有一个对象实例
	 * @see BeanDefinition#SCOPE_PROTOTYPE 允许Bean的定义可以被实例化多次(每次调用都创建一个实例)
	 * request 一次HTTP请求中，每个BeanDefinition对应一个实例，仅在基于Web的Spring上下文中才有效
	 * session 一个HTTP会话中，每个BeanDefinition对应一个实例，仅在基于Web的Spring上下文中才有效
	 * global-session 一个全局会话中，每个BeanDefinition对应一个实例，仅在基于Web的Spring上下文中才有效
	 */
	@Nullable
	 ...
	 * 是否延迟初始化
	 *
	 * @see DefaultListableBeanFactory#preInstantiateSingletons() 会根据lazyInit判断是否进行初始化
	 */
	@Nullable
	...
	/**
	 * Bean初始化依赖的Bean的名称，Bean Factory将会确保这些Bean被初始化。
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * 是否该bean适合自动注入到其他Bean
	 */
	...
	/**
	 * 指定一个用于创建bean实例的回调，作为一个替代去声明工厂方法
